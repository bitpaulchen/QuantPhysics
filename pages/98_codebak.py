...
# --- 1. 质点模型 ---
st.header("质点模型")
st.markdown("用来代替物体的有质量的点，是理想化模型（现实中不存在）。针对某个研究问题，当物体的形状和大小对研究问题的影响可忽略时，可将其简化为质点。")

# --- 2. 参考系 ---
st.header("参考系")

st.markdown("描述运动必须指明**相对于什么**。被选作参照标准的物体称为参考系。")

st.markdown("- 同一运动，选取不同参考系，描述结果不同（运动的相对性）")
st.markdown("- 通常默认选取**地面**作为参考系")
st.markdown("- 建立坐标系后，可用坐标定量描述位置")
